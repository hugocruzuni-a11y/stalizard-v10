...
st.set_page_config(
    page_title="STARLINE V133 - FULL MIND", 
    layout="wide", 
...
# --- SIDEBAR COCKPIT ---
with st.sidebar:
    st.markdown("<h2 style='color:#00FF88; font-size:22px; font-weight:700;'>🏛️ ORACLE V133</h2>", unsafe_allow_html=True)
    ...
    hgf = c_gf.number_input("H-GF", value=9, step=1, format="%d")
    hga = c_gf.number_input("H-GA", value=7, step=1, format="%d")
    agf = c_ga.number_input("A-GF", value=12, step=1, format="%d")
    aga = c_ga.number_input("A-GA", value=10, step=1, format="%d")

    st.markdown("<hr>", unsafe_allow_html=True)
    c1, cx, c2 = st.columns(3)
    m1 = c1.number_input("1", value=1.90); mx = cx.number_input("X", value=4.00); m2 = c2.number_input("2", value=3.35)
    
    o25_i = st.number_input("O2.5 ODD", 1.33); m_ob = st.number_input("BTTS ODD", 1.32)
    ah_h = st.number_input("AH 0.0 (H) ODD", 1.33)
...
# --- RESULTS INTERFACE ---
if not run:
    st.markdown("<div style='text-align:center; padding-top:150px; opacity:0.1;'><h1>ORACLE V133</h1><p>SOVEREIGN FULL-MIND BUILD</p></div>", unsafe_allow_html=True)
else:
    # --- MATH ENGINE ---
    ...
    with c_ins1:
        ...
        Trend: <b>{"OPEN GAME" if (lh+la) > 2.6 else "TIGHT DEFENSIVE BATTLE"}</b><br>
        Volatility Index: <b>{((lh*la)**0.5):.2f}</b><br>
        Poisson Scale: Bivariate Dixon-Coles correction applied.
    ...
    fig_p.update_layout(title="POISSON PROBABILITY DISTRIBUTION OVERLAP", paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font_color="white", height=350)
    st.plotly_chart(fig_p, use_container_width=True)
    
    st.markdown(f"""
        <div class="intel-card">
            <b style="color:#00FF88;">🌐 ORACLE INSIGHTS // GRÁFICO DE DENSIDADE:</b><br>
            <span style="color:#CBD5E1; line-height:1.6;">
            A curva <span style="color:#00FF88; font-weight:700;">VERDE ({h_n})</span> e a curva <span style="color:#3B82F6; font-weight:700;">AZUL ({a_n})</span> mostram a probabilidade de cada equipa marcar X golos. 
            A interseção das áreas revela o equilíbrio do jogo. Picos mais altos indicam maior previsibilidade, enquanto curvas baixas e largas indicam alta incerteza e variância estatística.
            </span>
        </div>
    """, unsafe_allow_html=True)
