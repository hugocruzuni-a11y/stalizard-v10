...
# ==========================================
# 1. SETUP DE ALTA PERFORMANCE (WALL STREET)
# ==========================================
...
# ==========================================
# 2. MOTOR OMNISCENCE E INTEGRAÇÃO DE MERCADOS PRO
# ==========================================
...
def get_smart_odds(fixture_id, model_probs): 
    ...
    market_odds["_source"] = "Pinnacle/Asian Proxy (Reconstructed)"
    return market_odds
...
<div class="ticker-wrap">
    ...
    <div class="ticker">
        ...
        <span class="ticker-item">EXECUTION LATENCY: 8ms</span>
    </div>
</div>
""", unsafe_allow_html=True)
...
# --- EXECUÇÃO (DIREITA) MATRIZ DE SINDICATO ---
with c_main:
    if m_sel:
        ...
        <div class="header-row">
            ...
            <div style="text-align:center;">Expected Value (Edge)</div>
        ...
        for mkt, m_odd in market_data.items():
            ...
            if m_odd > 1.01 and p_real > 0:
                ...
                <div class="order-row" style="background: {bg_highlight};">
                    ...
                    <div class="order-cell" style="text-align:right; font-weight:bold; color:#FFF;">{m_odd:.2f}</div>
                ...
                with col_btn:
                    if edge > 0.01:
                        ...
                        if st.button(f"BUY €{stake:,.0f}", key=f"buy_{mkt}", use_container_width=True):
                            ...
                            st.toast(f"Order executed in Asian Market.", icon="⚡")
                            safe_rerun()
