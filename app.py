...
st.set_page_config(
    page_title="STARLINE V132 - VISION", 
    layout="wide", 
...
# --- SIDEBAR COCKPIT ---
with st.sidebar:
    st.markdown("<h2 style='color:#00FF88; font-size:22px; font-weight:700;'>🏛️ ORACLE V132</h2>", unsafe_allow_html=True)
...
# --- RESULTS INTERFACE ---
if not run:
    st.markdown("<div style='text-align:center; padding-top:150px; opacity:0.1;'><h1>ORACLE V132</h1><p>THE QUANTUM VISION BUILD</p></div>", unsafe_allow_html=True)
else:
    # --- MATH ENGINE (SKELAM, DIXON-COLES & RISK ANALYTICS) ---
    lh = max(0.01, (hgf/5 * aga/5)**0.5); la = max(0.01, (agf/5 * hga/5)**0.5)
    
    # Tournament Tactical Shift
    ...
    # MATRIX COM SOMBREAMENTO ATÓMICO (#FF8C00)
    ...
    def get_row_color(edge):
        ...
        elif edge > 0.05: return 'rgba(255, 140, 0, 0.25)' # Laranja Atómico
    ...
    xr = list(range(7))
    fig_p = go.Figure()
    # Gráfico de Densidade Restaurado
    ...
    fig_p.update_layout(title="POISSON PROBABILITY DISTRIBUTION (GOALS)", paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font_color="white", height=350)
    st.plotly_chart(fig_p, use_container_width=True)
    
    # EXPLICAÇÃO ANALÍTICA DO GRÁFICO
    st.markdown(f"""
        <div class="intel-card">
            <b style="color:#00FF88;">🌐 ORACLE INSIGHTS // GRÁFICO DE DENSIDADE DE GOLOS:</b><br>
            <span style="color:#CBD5E1; line-height:1.6;">
            A curva <span style="color:#00FF88; font-weight:700;">VERDE ({h_n})</span> representa o fluxo de golos provável da equipa da casa. O pico indica o score mais expectável.<br>
            A curva <span style="color:#3B82F6; font-weight:700;">AZUL ({a_n})</span> representa o fluxo de golos da equipa visitante.<br>
            A área de sobreposição (<span style="color:white; font-weight:700;">OVERLAP</span>) indica a probabilidade de empate e equilíbrio táctico. Quanto mais distantes os picos, maior a vantagem analítica de uma equipa sobre a outra com base nas métricas de performance (GF/GA).
            </span>
        </div>
    """, unsafe_allow_html=True)
