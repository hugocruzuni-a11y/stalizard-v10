...
st.set_page_config(
    page_title="STARLINE V119 - PRECISION", 
    layout="wide", 
    initial_sidebar_state="expanded"
)

# 2. Sovereign Precision CSS
st.markdown("""
    <style>
    ...
    [data-testid="stSidebar"] .stTextInput input {
        ...
        font-weight: 400 !important; /* RETIRADO O NEGRITO */
...
# --- SIDEBAR COCKPIT ---
with st.sidebar:
    st.markdown("<h2 style='color:#00FF88; font-size:24px; font-weight:800;'>🏛️ ORACLE V119</h2>", unsafe_allow_html=True)
    ...
    o05 = st.number_input("OVER 0.5 GOALS ODD", 1.05)
    ...
    ah_h = st.number_input("AH 0.0 (HOME)", 1.33)
    ah_a = st.number_input("AH 0.0 (AWAY)", 1.85)
...
# --- RESULTS ---
if not run:
    st.markdown("<div style='text-align:center; padding-top:150px; opacity:0.1;'><h1>ORACLE V119</h1><p>AWAITING DATA COMMAND</p></div>", unsafe_allow_html=True)
else:
    # ENGINE
    ...
    st.markdown(f"<h1 style='letter-spacing:-4px; font-size:55px; margin:0;'>{h_n} <span style='color:#00FF88;'>vs</span> {a_n}</h1>", unsafe_allow_html=True)
    ...
    # MATRIX - SEM SCROLL E CENTRADA
    df = pd.DataFrame(mkts, columns=["Market", "Prob", "Odd"])
    df["Fair"] = 1/df["Prob"]; df["Edge"] = (df["Prob"] * df["Odd"]) - 1
    
    # Calculo da altura para evitar scroll (mercados * altura da linha + header)
    dynamic_height = (len(mkts) * 40) + 60
