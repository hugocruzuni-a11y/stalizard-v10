...
# --- MOTOR DE API DIRETA ---
api_key = "8171043bf0a322286bb127947dbd4041" # <-- COLA A TUA CHAVE AQUI
...
def get_pre_match_odds(fixture_id):
    """Extrai odds reais das casas de apostas (Bet365 / Bookmaker ID 8)"""
    url = f"https://{api_host}/odds"
    querystring = {"fixture": fixture_id, "bookmaker": 8} 
    odds_dict = {"1": 2.00, "X": 3.00, "2": 3.00, "O25": 1.90, "U25": 1.90, "BTTS": 1.90} # Fallbacks
    try:
        response = requests.get(url, headers=headers, params=querystring)
        ...
        if data:
            ...
            for mkt in markets:
                ...
                elif mkt['name'] == 'Goals Over/Under' and '2.5' in mkt['values'][0]['value']:
                    odds_dict["O25"] = float(mkt['values'][0]['odd'])
                    odds_dict["U25"] = float(mkt['values'][1]['odd'])
...
with st.sidebar:
    ...
    match_selected = None
    live_odds = {"1": 2.00, "X": 3.20, "2": 3.00, "O25": 1.85, "U25": 1.85, "BTTS": 1.75} # Valores padrão
    ...
    if not fixtures:
        ...
        # Puxa as odds automaticamente ao selecionar o jogo!
        with st.spinner('A extrair Odds do Mercado...'):
    ...
    st.markdown("<hr>", unsafe_allow_html=True)
    st.markdown("<p style='color:#475569; font-size:0.65rem; font-weight:700;'>03 // BOOKMAKER ODDS (Auto-Filled)</p>", unsafe_allow_html=True)
    c1, cx, c2 = st.columns(3)
    # Os inputs agora são pré-preenchidos com a API!
    ...
    o25_i = col_g1.number_input("O2.5 Odd", value=live_odds["O25"], format="%.2f")
    u25_i = col_g2.number_input("U2.5 Odd", value=live_odds["U25"], format="%.2f")
    m_ob = st.number_input("BTTS (YES) Odd", value=live_odds["BTTS"], format="%.2f")
    
    st.markdown("<br>", unsafe_allow_html=True)
    run_scan = st.button("🚀 EXECUTE ALPHA SCAN")

# --- RESULTS INTERFACE (O resto mantém-se igual!) ---
if not run_scan or not match_selected:
    st.markdown("<div style='text-align:center; padding-top:150px; opacity:0.1;'><h1>ORACLE V140 PRO</h1><p>FULLY AUTOMATED PRE-MATCH ENGINE</p></div>", unsafe_allow_html=True)
else:
    # --- PRO MATH ENGINE (DIXON-COLES + LIVE API) ---
    with st.spinner('A processar Quant Engine e a calibrar a Matriz de Dixon-Coles...'):
    ...
    # APLICAÇÃO DO ALGORITMO DE DIXON-COLES
    for x in range(2):
        for y in range(2):
            if x == 0 and y == 0:
                correction = 1 - (lh * la * rho_param)
            elif x == 0 and y == 1:
                correction = 1 + (lh * rho_param)
            elif x == 1 and y == 0:
                correction = 1 + (la * rho_param)
            elif x == 1 and y == 1:
                correction = 1 - rho_param
            else:
                correction = 1.0
    ...
    st.markdown(f"<p style='color:#64748B; font-size:0.8rem; text-transform:uppercase;'>{league_name} // Match ID: {match_id} // Full API Data Sync</p>", unsafe_allow_html=True)
    ...
    best = sorted([(n, p, b, (p*b)-1) for n, p, b in mkts if p > 0 and b > 1.01], key=lambda x: x[3], reverse=True)[0]
    kelly = max(0, (best[3] / (best[2] - 1)) * 0.25) 

    with col_res:
        st.markdown(f"""<div class="advisor-seal"><h1 class="advisor-title">{best[0]}</h1><p class="advisor-subtitle">ALPHA EDGE: {best[3]:+.1%} | Q-KELLY STAKE: {kelly:.2%}</p></div>""", unsafe_allow_html=True)
    with col_risk:
        st.markdown(f"""<div class="risk-card"><b style="color:#00FF88; letter-spacing:1px; font-size:0.65rem;">🛡️ INSTITUTIONAL CONFIDENCE</b><h2 style="margin:5px 0; font-size:2rem; font-weight:700;">{conf_score:.1f}%</h2><p style="color:#64748B; font-size:0.75rem; margin:0;">API LIVE ODDS & DIXON-COLES</p></div>""", unsafe_allow_html=True)
