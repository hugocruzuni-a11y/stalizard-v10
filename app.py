...
st.set_page_config(
    page_title="STARLINE V134 - COMPLETION", 
    layout="wide", 
...
# --- SIDEBAR COCKPIT ---
with st.sidebar:
    st.markdown("<h2 style='color:#00FF88; font-size:22px; font-weight:700;'>🏛️ ORACLE V134</h2>", unsafe_allow_html=True)
...
# --- RESULTS INTERFACE ---
if not run:
    st.markdown("<div style='text-align:center; padding-top:150px; opacity:0.1;'><h1>ORACLE V134</h1><p>THE SOVEREIGN COMPLETION</p></div>", unsafe_allow_html=True)
else:
    # --- MATH ENGINE ---
    ...
    with col_risk:
        risk_color = "#00FF88" if kelly > 0.03 else "#FF8C00" if kelly > 0.01 else "#FF4D4D"
        st.markdown(f"""<div class="risk-card"><b style="color:{risk_color}; letter-spacing:1px; font-size:0.65rem;">🛡️ QUANTUM ALLOCATION</b><h2 style="margin:5px 0; font-size:2rem; font-weight:700;">{kelly:.1%}</h2><p style="color:#64748B; font-size:0.75rem; margin:0;">STAKE ADVISED (HALF-KELLY)</p></div>""", unsafe_allow_html=True)

    # IA INSIGHTS
    c_ins1, c_ins2 = st.columns(2)
    with c_ins1:
        st.markdown(f"""<div class="intel-card"><b style="color:#00FF88;">🧠 AI TACTICAL ANALYSIS</b><br>
        <span style="color:#CBD5E1; line-height:1.6;">Trend: <b>{"OPEN GAME" if (lh+la) > 2.6 else "TIGHT DEFENSE"}</b><br>Volatility: <b>{((lh*la)**0.5):.2f}</b><br>Dixon-Coles bivariate scale applied.</span></div>""", unsafe_allow_html=True)
    with c_ins2:
        ...
        <span style="color:#CBD5E1;">1. {idx[0][2]}-{idx[1][2]} ({mtx[idx[0][2], idx[1][2]]:.1%}) | 2. {idx[0][1]}-{idx[1][1]} ({mtx[idx[0][1], idx[1][1]]:.1%})</span></div>""", unsafe_allow_html=True)

    # MATRIX COM SOMBREAMENTO ATÓMICO
    ...
    st.markdown(f"""<div class="intel-card"><b style="color:#00FF88;">🌐 ORACLE INSIGHTS // GRÁFICO:</b><br><span style="color:#CBD5E1; font-size:0.85rem;">As curvas mostram a distribuição de probabilidade de golos. Onde as áreas se cruzam, reside o equilíbrio estatístico do confronto.</span></div>""", unsafe_allow_html=True)
