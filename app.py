...
st.set_page_config(
    page_title="STARLINE V137 - THE FINAL TITAN", 
    layout="wide", 
...
# --- SIDEBAR COCKPIT ---
with st.sidebar:
    st.markdown("<h2 style='color:#00FF88; font-size:22px; font-weight:700;'>🏛️ ORACLE V137</h2>", unsafe_allow_html=True)
    ...
    o25_i = st.number_input("O2.5", 1.65); o35_i = st.number_input("O3.5", 2.20)
    ...
    st.write("SPECIALS")
    m_ob = st.number_input("BTTS (YES)", 1.55)
    ah_h = st.number_input("AH 0.0 (H)", 1.55); ah_a = st.number_input("AH 0.0 (A)", 2.40)
    
    st.markdown("<br>", unsafe_allow_html=True)
    run = st.button("🚀 EXECUTE TITAN SCAN")
    st.button("🗑️ RESET ENGINE", on_click=reset)

# --- RESULTS INTERFACE ---
if not run:
    st.markdown("<div style='text-align:center; padding-top:150px; opacity:0.1;'><h1>ORACLE V137</h1><p>THE FINAL TITAN BUILD</p></div>", unsafe_allow_html=True)
else:
    # --- 🧠 MATH ENGINE (BIVARIATE DIXON-COLES & 95% CONFIDENCE) ---
    ...
    # Statistical Confidence Level (Z-Score 1.96 for 95%)
    std_dev = np.sqrt(lh + la)
    margin_of_error = 1.96 * (std_dev / np.sqrt(100))
    confidence_score = max(0, min(100, 100 * (1 - (margin_of_error / (lh + la)))))
    ...
    with col_risk:
        st.markdown(f"""<div class="risk-card"><b style="color:#00FF88; letter-spacing:1px; font-size:0.65rem;">🛡️ STATISTICAL CONFIDENCE (95% CI)</b><h2 style="margin:5px 0; font-size:2rem; font-weight:700;">{confidence_score:.1%}%</h2><p style="color:#64748B; font-size:0.75rem; margin:0;">HALF-KELLY OPTIMIZATION ACTIVE</p></div>""", unsafe_allow_html=True)
    ...
    with c_ins1:
        ...
        Volatility: <b>{((lh*la)**0.5):.2f}</b> | Confidence: <b>95% Alpha</b><br>
        Dixon-Coles bivariate adjustment applied for low-score accuracy.</span></div>""", unsafe_allow_html=True)
    with c_ins2:
        ...
        1. {idx[0][2]}-{idx[1][2]} ({mtx[idx[0][2], idx[1][2]]:.1%}) | 2. {idx[0][1]}-{idx[1][1]} ({mtx[idx[0][1], idx[1][1]]:.1%})<br>
        3. {idx[0][0]}-{idx[1][0]} ({mtx[idx[0][0], idx[1][0]]:.1%})
        </span></div>""", unsafe_allow_html=True)

    # --- MATRIX SOMBREAMENTO ATÓMICO (#FF8C00) ---
    ...
    fig_p.update_layout(title="POISSON DENSITY DISTRIBUTION", paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font_color="white", height=300)
    st.plotly_chart(fig_p, use_container_width=True)
    
    st.markdown(f"""<div class="intel-card"><b style="color:#00FF88;">🌐 ORACLE INSIGHTS // GRÁFICO DE DENSIDADE:</b><br><span style="color:#CBD5E1; font-size:0.85rem; line-height:1.5;">O gráfico cruza a expectativa de golos de ambas as equipas com um intervalo de confiança de 95%. A zona de sobreposição representa o equilíbrio tático. Picos divergentes indicam uma "Edge" analítica clara no mercado de Match Odds.</span></div>""", unsafe_allow_html=True)
