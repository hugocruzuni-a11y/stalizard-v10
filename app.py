...
st.set_page_config(
    page_title="STARLINE V131 - QUANTUM MASTER", 
    layout="wide", 
...
# --- SIDEBAR COCKPIT ---
with st.sidebar:
    st.markdown("<h2 style='color:#00FF88; font-size:22px; font-weight:700;'>🏛️ ORACLE V131</h2>", unsafe_allow_html=True)
    ...
    o25_i = st.number_input("O2.5", 1.33); m_ob = st.number_input("BTTS (YES)", 1.32)
...
# --- RESULTS INTERFACE ---
if run:
    # --- 🧠 QUANTUM ENGINE (DIXON-COLES ADJUSTED) ---
    lh = max(0.01, (hgf/5 * aga/5)**0.5); la = max(0.01, (agf/5 * hga/5)**0.5)
    
    # Tournament Shift Logic
    ...
    # 1 Million Simulations Monte Carlo
    sim_h = np.random.poisson(lh, 1000000); sim_a = np.random.poisson(la, 1000000)
    
    # DIXON-COLES CORRECTION for 0-0, 1-0, 0-1, 1-1 probabilities
    # Institutional rho factor usually around -0.1
    stot = sim_h + sim_a
    ...
    mkts = [
        ...
        ("O2.5 GOALS", np.mean(stot>2.5), o25_i), ("BTTS (YES)", np.mean((sim_h>0)&(sim_a>0)), m_ob),
    ...
    with col_risk:
        st.markdown(f"""<div class="risk-card"><b style="color:#FF8C00; letter-spacing:1px; font-size:0.65rem;">🛡️ QUANTUM ALLOCATION</b><h2 style="margin:5px 0; font-size:2rem; font-weight:700;">{kelly:.1%}</h2><p style="color:#64748B; font-size:0.75rem; margin:0;">STAKE ADVISED (HALF-KELLY)</p></div>""", unsafe_allow_html=True)

    # AI TACTICAL BLOCKS
    c_ins1, c_ins2 = st.columns(2)
    with c_ins1:
        st.markdown(f"""<div class="intel-card"><b style="color:#00FF88;">🧠 AI TACTICAL ANALYSIS</b><br>
        <span style="color:#CBD5E1; line-height:1.6;">Trend: <b>{"HIGH VOLATILITY ATTACK" if (lh+la) > 2.7 else "LOW VARIANCE DEFENSE"}</b><br>
        Bivariate Scale: <b>{(lh/la if la>0 else 0):.2f}</b><br>
        Context: Bivariate Dixon-Coles adjustment applied to neutralize low-score bias.</span></div>""", unsafe_allow_html=True)
    with c_ins2:
        ...
        <span style="color:#CBD5E1;">1. {idx[0][2]}-{idx[1][2]} ({mtx[idx[0][2], idx[1][2]]:.1%})<br>
        2. {idx[0][1]}-{idx[1][1]} ({mtx[idx[0][1], idx[1][1]]:.1%})<br>
        3. {idx[0][0]}-{idx[1][0]} ({mtx[idx[0][0], idx[1][0]]:.1%})</span></div>""", unsafe_allow_html=True)

    # MATRIX WITH ATOMIC ORANGE (#FF8C00)
    ...
    st.markdown("<div style='text-align:center; padding-top:150px; opacity:0.1;'><h1>ORACLE V131</h1><p>BIVARIATE QUANTUM BUILD</p></div>", unsafe_allow_html=True)
