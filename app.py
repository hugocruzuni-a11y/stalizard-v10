...
# ==========================================
# 3. SISTEMA DE LOGIN E UI PRINCIPAL
# ==========================================
def login_screen():
    st.markdown("<div style='text-align:center; padding-top:100px;'><h1 style='color:#00FF88; font-size:4rem; letter-spacing:-2px;'>ORACLE V140</h1><p style='color:#94A3B8;'>APEX QUANT ANALYTICS</p></div>", unsafe_allow_html=True)
...
def main_app():
    ...
    # Cálculo do Bankroll em tempo real
    df_temp = st.session_state.bet_history
    lucro_real_total = 0
    if not df_temp.empty:
        for idx, row in df_temp.iterrows():
            if row['Estado'] == 'Ganha':
                lucro_real_total += row['Stake (€)'] * (row['Odd Comprada'] - 1)
            elif row['Estado'] == 'Perdida':
                lucro_real_total -= row['Stake (€)']
    ...
    with tab1:
        if not m_sel:
            ...
            if value_bets:
                ...
                with col_btn:
                    ...
                    if st.button("📥 REGISTAR", key="reg_bet", use_container_width=True):
                        ...
                        st.toast("Aposta gravada na Caixa Forte!", icon="✅")
            else:
            ...
            with col_chart:
                ...
                fig.update_layout(title="📊 PROBABILIDADE DE GOLOS", height=230, margin=dict(l=20,r=20,t=40,b=20), paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font=dict(color="#FFF"))
            ...
            if not fix_data: st.warning("Sem jogos para a liga selecionada.")
                ...
                for i, f in enumerate(fix_data):
                    try:
                        ...
                        status_text.markdown(f"🔍 **A analisar:** {home} vs {away}...")
                        ...
                        if odds.get("1", 0) > 1.10:
                            ...
                            for m_name, prob_data, odd in scan_mkts:
                                ...
                                if 1.40 <= odd <= 4.00 and p_win > 0:
                                    ...
                                    if edge > 0.045: 
                                        portfolio.append({
                                            "Jogo": f"{home} vs {away}", "Aposta": m_name, "Certeza": p_win, 
                ...
                if portfolio:
                    ...
                    df_port["Stake (€)"] = df_port["Kelly_Raw"] * bankroll_atual * min(0.15 / sum_kelly, 1.0) if sum_kelly > 0 else 0
                    
                    st.dataframe(df_port[["Jogo", "Aposta", "Certeza", "Odd Casa", "Lucro Extra", "Stake (€)"]].style.format({
                        "Certeza": "{:.1%}", "Odd Casa": "{:.2f}", "Lucro Extra": "{:+.1%}", "Stake (€)": "{:.2f}€"
                    }))
                else:
